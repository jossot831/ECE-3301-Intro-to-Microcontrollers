...
char NS_PED_SW = 0;                     // RAM variable for NS Pedestrian Switch used in INT0
char EW_PED_SW = 0;                     // RAM variable for NS Pedestrian Switch used 
...
void main(void)
{
    ...
    {                                           // Do nothing,       
        ...
        {     
            NS_PED_SW = 0;                      // calls INT0
            EW_PED_SW = 0;                      // calls INT1
...
void init_IO()
{
    TRISA = 0x31;                               // makes bits A0 A4 A5
...
{ 
    ...
    for(char i = Num_Sec-1;i>0; i--)
    {
        ...
        Wait_One_Second_With_Beep();                // hold the number on LCD for 1 second
    }
...
void update_LCD_color(char direction, char color)
{
    ...
    if (color == Color_Off)     //if Color off  leave outline
    {
    ...
    if (color == Color_Red)     //if the color is red fill the red circle with red
    {
    ...
    if (color == Color_Green)     //if the color is green fill the green circle with green
    {
        fillCircle(XRED, Circle_Y, Circle_Size, ST7735_BLACK);
        fillCircle(XYEL, Circle_Y, Circle_Size, ST7735_BLACK);
...
{
   switch (direction)                   //update traffic light 
   {
      case EW:        
...
void update_LCD_PED_Count(char direction, char count)
{
   ...
   {
      case EW:       
        PED_EW_Count[0] = count/10  + '0';          //  PED upper digit
        PED_EW_Count[1] = count%10  + '0';          //  PED lower digit
        drawtext(PED_Count_X, PED_EW_Count_Y, PED_EW_Count, EW_Color, ST7735_BLACK, TS_2);        
        break;

      case NS:       
        PED_NS_Count[0] = count/10  + '0';          // PED upper digit
...
{
    int nStep = get_full_ADC();         // # of steps for analog conversion
    volt = nStep * 5 /1024.0;           // gets the voltage in Volts, using 5V as reference s instead of 4, also divide by 1024 
    SW_MODE = volt < 2.5 ? 1:0;         // Mode = 1, Day_mode, Mode = 0 Night_mode
    ...
    if (SW_EWPED == 0) SW_EWPED_Txt[0] = '0'; else SW_EWPED_Txt[0] = '1';      // text at bottom of screen to switch states
    if (SW_EWLT == 0) SW_EWLT_Txt[0] = '0'; else SW_EWLT_Txt[0] = '1';         // text at bottom of screen to switch states
    if (SW_NSPED == 0) SW_NSPED_Txt[0] = '0'; else SW_NSPED_Txt[0] = '1';      // text at bottom of screen to switch states
    if (SW_NSLT == 0) SW_NSLT_Txt[0] = '0'; else SW_NSLT_Txt[0] = '1';         // text at bottom of screen to switch states
