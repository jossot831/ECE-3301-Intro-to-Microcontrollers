...
#define SEC_LED         PORTDbits.RD7   //  SEC_LED as PORTD bit RD7

#define NS_RED          PORTAbits.RA1   //  NS_RED as  bit RA1                
#define NS_GREEN        PORTAbits.RA2   //  NS_GREEN as bit RA2

#define NSLT_RED PORTBbits.RB4          // NSLT_RED as bit B4
#define NSLT_GREEN PORTBbits.RB5        // NSLT_GREEN as bit B5

#define EW_RED PORTBbits.RB6            // EW_RED as bit B6
#define EW_GREEN PORTBbits.RB7          // EW_RED as bit B7

#define EWLT_RED PORTEbits.RE0          // EWLT_RED as bit E0
#define EWLT_GREEN PORTEbits.RE2        // EWLT_GREEN as bit E2

#define NS_LT_SW PORTAbits.RA5          // NS_LT_SW as bit A5
#define EW_LT_SW PORTAbits.RA4          // EW_LT_SW as bit A4
#define MODE_LED PORTEbits.RE1          // MODE_LED as bit RE1
...
#include "ST7735_TFT.c"                 // needed to enable TFT functions           
...
char dir;
char Count;                             //  variable for Second Count
char PED_Count;                         //  variable for Second Pedestrian Count

char SW_EWPED;                          //  variable for EWPED Switch
char SW_EWLT;                           //  variable for EWLT Switch
char SW_NSPED;                          //  variable for NSPED Switch
char SW_NSLT;                           //  variable for NSLT Switch

char NS_PED_SW = 0;                     //  variable for NSPED used in INT0
char EW_PED_SW = 0;                     //  variable for EWPED used in INT1

char SW_MODE;                           //  variable for Mode Light Sensor
int MODE;                               //  variable for Mode
char direction;                         //  variable for direction
float volt;                             //  variable for voltage from light sensor   

char FLASHING_REQUEST = 0;              //  variable for flashing request
char FLASHING = 0;                      //  variable for flashing
...
void Do_Flashing(void);                 // end prototyping
...
void main(void)
{
    ...
    {                                           // Do nothing,       
        ...
        {     
            NS_PED_SW = 0;                      // clears INT0
            EW_PED_SW = 0;                      // clears INT1
            Night_Mode();                       // calls Night_Mode() function
        }
        if (FLASHING_REQUEST == 1)              
        {
            FLASHING_REQUEST = 0;               // clears the flag  
            Do_Flashing();                      // calls Do_Flashing() function
        }
    }
}
...
void init_IO()
{
    TRISA = 0x31;                               // makes bits A0 A4 A5 inputs
                                                // makes all other bits of PORTA as output
    TRISB = 0x07;                               // makes b0 b1 b2 as inputs
...
{ 
    ...
    if (direction == NS)                        
        NS_PED_SW = 0;                          // clears the flag for NS PED switch
    else 
        EW_PED_SW = 0;                          // clears the flag for EW PED switch
...
void INT0_ISR()
{
    ...
    if (MODE == 0)
        NS_PED_SW = 0;                          // clear software INT0_flag
    else
...
void INT1_ISR()
{
    ...
    if (MODE == 0)
        EW_PED_SW = 0;                          // clear software INT1_flag
    else
...
void Do_Flashing()
{
    FLASHING = 1;                               // sets flashing to 1
    update_LCD_misc();
    
    while(FLASHING == 1)
    {
        if(FLASHING_REQUEST == 1)               // gets out of the loop if user presses INT2 switch
        {
            FLASHING_REQUEST = 0;               // clears FLASHING_REQUEST
            FLASHING = 0;                       // clears FLASHING
            update_LCD_misc();
        }
        else
        {                                       // sets all directions to RED color
            Set_NS_LT(RED);                       
            Set_EW(RED);
            Set_EW_LT(RED);
            Set_NS(RED);     
            
            Wait_One_Second();                  // holds the color one second
                                                // clears all directions with OFF color
            Set_NS_LT(OFF);      
            Set_EW(OFF);
            Set_EW_LT(OFF);
            Set_NS(OFF);     
            
            Wait_One_Second();                  // leaves the OFF color for one second
        }
    }
}
