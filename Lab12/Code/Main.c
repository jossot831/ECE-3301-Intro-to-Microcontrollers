...
char array1[21]={0xa2,0x62,0xe2,0x22,0x02,0xc2,0xe0,0xa8,0x90,0x68,0x98,0xb0,0x30,0x18,0x7a,0x10,0x38,0x5a,0x42,0x4a,0x52}; // array for buttons
...
{ 
    ...
    ADCON1=0x0E;
    TRISA = 0x01;                                                               // A0 as input
    TRISB = 0x07;                                                               // B0-B2 as input
    TRISC = 0x03;                                                               // c0-c1 as input
    TRISD = 0x00;                   
    TRISE = 0x00;
    PORTE = 0x00;
...
void main() 
{
    ...
    while (1)
    {
        ...
        {
            tempSecond = second;
            rpm = get_RPM();                                                    // gets rpm
            volt = read_volt();                                                 // gets volt
            ...
            printf ("%02x:%02x:%02x %02x/%02x/%02x",hour,minute,second,month,day,year);     // prints on tera
            ...
            Monitor_Heater();
            Test_Alarm();
            Set_RPM_RGB(rpm);                                                   // D2 RGB LED
            Update_Screen();
        }
        
        if (check_for_button_input() == 1)                                      //  does function based off controller input
        {
...
{
    // add code here
    if (alarm_mode == 0 && ALARMEN == 1)                                        // alarm_mode is 0 and ALARMEN = 1. This means that no alarm
                                                                                // operation was on and now it is activated
    {
        DS3231_Turn_On_Alarm();
        alarm_mode = 1;
        MATCHED = 0;
    }
    else if (alarm_mode == 1 && ALARMEN == 0)                                   // forces the alarm mode to be disabled
    ...
    else if (alarm_mode == 1 && ALARMEN == 1)                                   // the program is waiting for the alarm time to match with the actual time
    {
        if (RTC_ALARM_NOT == 0)
        {
            MATCHED = 1;
