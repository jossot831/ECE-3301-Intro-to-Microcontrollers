...
void Do_Setup_Alarm_Time(void)
{
    ...
    {   
         if (check_for_button_input() == 1)
         {
             ...
             {
                Do_Beep_Good();                                         // does commands based off input
                if (found == Prev) Go_Prev_Alarm_Field();               // prev alarm
                if (found == Next) Go_Next_Alarm_Field();               // next alarm
                if (found == Play_Pause) Do_Save_New_Alarm_Time();      // new alarm time
                if (found == Minus) Decrease_Alarm_Time();              // decrease alarm
                if (found == Plus) Increase_Alarm_Time();               // increase alarm
                if (found == EQ) Exit_Setup_Alarm_Time();               // exit
                found = 0xff;
             }
             else
             {
...
void Increase_Alarm_Time()
{
            switch (Select_Alarm_Field)                                         // Increment the proper field
            {
                ...
                case 2:
                    setup_alarm_second++;                                       // increments second
...
{       
            switch (Select_Alarm_Field)                                         // Decrement the proper field
            {
                ...
                case 2:
                    if (setup_alarm_second == 0) setup_alarm_second = 59;       // decrements second
