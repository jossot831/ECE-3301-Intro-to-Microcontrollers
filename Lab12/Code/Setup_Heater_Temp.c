    ...
    while (In_Heater_Temp_Setup == 1)
    {
         if (check_for_button_input() == 1)
        {
             ...
             if (found >= Play_Pause && found <= EQ)
             {
                ...
                if (found == Play_Pause) Do_Save_New_Heater_Temp();             // controls heater temp by input, save new heater temp
                if (found == Minus) Decrease_Heater_Temp();                     // decrease heater temp
                if (found == Plus) Increase_Heater_Temp();                      // increase heater temp
                if (found == EQ) Exit_Setup_Heater_Temp();                      // exit heater temp
                found = 0xff;
             }
             else
             {
...
void Increase_Heater_Temp()
{
    ...
    if (setup_heater_set_temp > 110) setup_heater_set_temp = 110;               //  check if it is greater than 110. If so, limit to 110
...
{
    char dc_char1 = (setup_heater_set_temp/100) % 10;
