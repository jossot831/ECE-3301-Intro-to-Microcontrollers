...
void Do_Setup_Time(void)
{
    ...
    setup_year = bcd_2_dec(year);                                               // end of converting bcd to integer
    ...
    {   
         if (check_for_button_input() == 1)
         {
             ...
             {
                Do_Beep_Good(); 
                if (found == Prev) Go_Prev_Field();                             // does functions based off input, prev
                if (found == Next) Go_Next_Field();                             // next
                if (found == Play_Pause) Do_Save_New_Time();                    // save new time
                if (found == Minus) Decrease_Time();                            // decrease time
                if (found == Plus) Increase_Time();                             // increase time
                if (found == EQ) Exit_Time_Setup();                             // exit time
                found = 0xff;
             }
             else
             {
...
void Increase_Time()
{
            switch (Select_Field)               // Increment the proper field
            {
                ...
                case 4:
                                                                                // increments day
                    setup_day++;
                    if (setup_day == 30) setup_day = 1; //UP TO WHAT DAY SHOULD WE GO
                    break;
                case 5:
                                                                                // increments year
                    setup_year++;
